...
fn main() {
    ...
    let vulkan_sdk_dir = std::env::var("VULKAN_SDK").expect("failed to find the environment variable VULKAN_SDK");
    ...
    // link
    println!("cargo:rustc-link-search=native={}", vulkan_bin_dir.display());
...
fn copy_dll(source_path: impl AsRef<std::path::Path>, target_dir: impl AsRef<std::path::Path>) {
    ...
    let filename = filename
        .expect(&format!("failed to get file_stem of \"{}\"", source_path.display()))
        .to_str()
        .expect(&format!("failed to convert OsStr to str of \"{}\"", source_path.display()));
    let filestem = format!("{}{}", filename, extension);

    let source_dir = source_path.parent().expect(&format!("failed to get parent of \"{}\"", source_path.display()));
    let source_path = source_dir.join(&filestem);
    let target_path = target_dir.join(&filestem);
    std::fs::copy(&source_path, &target_path).expect(&format!("failed to copy \"{}\" to \"{}\"", source_path.display(), target_path.display()));
}
