...
impl ICommand for Asset {
    ...
    fn explanation(level: ExplanationLevel) -> String {
        match level {
            ...
            ExplanationLevel::Detailed => {
                ...
                explanation
                    .push_str("Compiles the assets in use.\n");
                explanation.push('\n');
                explanation.push_str(&format!("{}\n", DECOMPILE));
                explanation
                    .push_str("Decompiles the ris_assets file.\n");
                explanation.push('\n');
                explanation.push_str(&format!("{}\n", CLEAN));
                explanation
                    .push_str("Cleans the imported assets.\n");
                explanation.push('\n');
                explanation.push_str(&format!("{}\n", IMPORT));
    ...
    fn run(args: Vec<String>, _target_dir: PathBuf) -> RisResult<()> {
        ...
        match command.as_str() {
            ...
            DECOMPILE => {
                asset_compiler::decompile(
                    asset_compiler::DEFAULT_COMPILED_FILE,
                    asset_compiler::DEFAULT_DECOMPILED_DIRECTORY,
                )
            },
            CLEAN => {
                asset_importer::clean(
