...
impl ICommand for Asset {
    ...
    fn explanation(level: ExplanationLevel) -> String {
        match level {
            ExplanationLevel::Short => format!("Compile, decompile or import assets."),
            ExplanationLevel::Detailed => {
                ...
                explanation.push_str(&format!("commands:\n"));
                explanation.push_str(&format!("\n"));
                explanation.push_str(&format!("{}\n", COMPILE));
                explanation.push_str(&format!("Compiles the <source> directory into the asset file <target>.\n"));
                explanation.push_str(&format!("default source: {}\n", asset_compiler::DEFAULT_ASSET_DIRECTORY));
                explanation.push_str(&format!("default target: {}\n", asset_compiler::DEFAULT_COMPILED_FILE));
                explanation.push_str(&format!("\n"));
                explanation.push_str(&format!("{}\n", DECOMPILE));
                explanation.push_str(&format!("Decompiles the asset file <source> into the directory <target>.\n"));
                explanation.push_str(&format!("default source: {}\n", asset_compiler::DEFAULT_COMPILED_FILE));
                explanation.push_str(&format!("default target: {}\n", asset_compiler::DEFAULT_DECOMPILED_DIRECTORY));
                explanation.push_str(&format!("\n"));
                explanation.push_str(&format!("{}\n", IMPORTALL));
                explanation.push_str(&format!("Recursively imports ALL files in directory <source> into the directory <target>.\n"));
    ...
    fn run(args: Vec<String>, _target_dir: PathBuf) -> RisResult<()> {
        ...
        let (command, source_target) = match parse_result {
            Ok(ok) => ok,
            Err(e) => return crate::util::command_error(
                &e,
                "asset",
                Self::args(),
        ...
        match command.as_str() {
            COMPILE => {
                ...
                match source_target {
                    Some((source, target)) => asset_compiler::compile(
                        source,
                        target,
                        compile_options,
                    ),
            ...
            DECOMPILE => match source_target {
                Some((source, target)) => asset_compiler::decompile(
                    source,
                    target,
                ),
