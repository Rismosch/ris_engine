...
pub fn from(value: Quat) -> Vec3 {
    ...
    let t1 = f32::atan2(m.2.0, m.2.2);
    let c2 = f32::sqrt(m.0.1 * m.0.1 + m.1.1 * m.1.1);
    let t2 = f32::atan2(-m.2.1, c2);
    let s1 = f32::sin(t1);
    let c1 = f32::cos(t1);
    let t3 = f32::atan2(s1 * m.1.2 - c1 * m.1.0, c1 * m.0.0 - s1 * m.0.2);
